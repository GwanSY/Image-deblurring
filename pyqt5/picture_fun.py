...
################运动模糊#####################
def yd(image):
    image = np.array(image,degree=12, angle=45)
...
def pinpu(image):
    ...
    fimg_norm = (255.0 / fimg.max() * (fimg - fimg.min())).astype(np.uint8)
    fimg_gray = Image.fromarray(fimg_norm, mode='L')
    # Convert the image to a QPixmap object
    qimg = ImageQt(fimg_gray)
